...
Определения переменных: статические, условные (`switch`) и диапазонные (`range`). Диапазон предоставляет
`item,index` для коллекций и массивов и `key,value` для карт.
...
### Безопасный доступ к членам

Используйте `?.`, если свойство может отсутствовать у объекта во время выполнения.
Отсутствующее свойство возвращает `null`, поэтому оператор можно сочетать с `default`:
...
Для Java Bean выражение `?.on` обращается к публичному полю `on` или к методу
без аргументов `getOn()` / `isOn()`. Тот же оператор поддерживает безопасный
вызов методов:
...
Если для переданных аргументов нет подходящего метода, результатом вызова будет
`null`. Безопасный доступ не скрывает исключения, выброшенные существующим getter-ом
или найденным методом.
